...
from session import get_auth_user, UnauthenticatedException
...
@router.get("/", response_class=HTMLResponse, dependencies=[Depends(get_auth_user)])
async def get_shop_page(request: Request):
    return templates.TemplateResponse("pages/shop.html", {"request": request})


@router.get("/cart", response_class=HTMLResponse, dependencies=[Depends(get_auth_user)])
